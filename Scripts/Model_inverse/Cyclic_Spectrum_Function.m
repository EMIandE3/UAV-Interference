...
function [f,alpha,CS1, CS_0, CS_Carrier] = Cyclic_Spectrum_Function(CSF_Method, fs, ad, MapN)
...
%     mesh(app.cyclic_spectrum_3_dmension_axis, f,alpha,abs(CS1))
%     xlabel(app.cyclic_spectrum_3_dmension_axis, 'f/Hz')
%     ylabel(app.cyclic_spectrum_3_dmension_axis, '\alpha/Hz')
%     title(app.cyclic_spectrum_3_dmension_axis, "循环谱三维结果图");
% 
%     alpha_line = -fs/2:fs/MapN:0;
%     z = CS1(:,end-1);
%     plot(app.cyclic_spectrum_CF_estimation_axis, alpha_line,abs(z));
%     xlabel(app.cyclic_spectrum_CF_estimation_axis, '\alpha Hz')
%     grid(app.cyclic_spectrum_CF_estimation_axis, "on")
%     title(app.cyclic_spectrum_CF_estimation_axis, "循环谱载频估计\alpha平面图");
% 
%     [~,b] = max(abs(z));
% 
%     z = CS1(:,round(MapN-1-(MapN/2-b)/2*2));
%     plot(app.cyclic_spectrum_CR_estimation_axis, alpha_line,abs(z));
%     xlabel(app.cyclic_spectrum_CR_estimation_axis, '\alpha Hz')
%     title(app.cyclic_spectrum_CR_estimation_axis, "循环谱码速估计\alpha平面图");
%     grid(app.cyclic_spectrum_CR_estimation_axis, "on")
